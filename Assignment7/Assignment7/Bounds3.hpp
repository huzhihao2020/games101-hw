...
inline bool Bounds3::IntersectP(const Ray &ray, const Vector3f &invDir,
  ...
  auto tx1 = std::min(txMin, txMax);
  auto tx2 = std::max(txMin, txMax);
  auto ty1 = std::min(tyMin, tyMax);
  auto ty2 = std::max(tyMin, tyMax);
  auto tz1 = std::min(tzMin, tzMax);
  auto tz2 = std::max(tzMin, tzMax);

  auto tin = std::max({tx1, ty1, tz1});
  auto tout = std::min({tx2, ty2, tz2});
